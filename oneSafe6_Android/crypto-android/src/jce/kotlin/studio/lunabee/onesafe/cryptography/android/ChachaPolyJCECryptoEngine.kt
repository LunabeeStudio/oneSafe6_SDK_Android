...
class ChachaPolyJCECryptoEngine @Inject constructor(
    ...
    override fun decrypt(cipherData: ByteArray, key: ByteArray, associatedData: ByteArray?): Result<ByteArray> = runCatching {
        ...
        val iv: ByteArray = try {
            cipherData.copyOfRange(0, NonceLength)
    ...
    private fun getCipher() = Cipher.getInstance(CipherTransformation)

    private fun getSecretKeySpec(key: ByteArray) = SecretKeySpec(key, KeyAlgorithm)

    companion object {
        private const val KeyAlgorithm = "ChaCha20"

        private const val CipherTransformation = "CHACHA20-POLY1305"
