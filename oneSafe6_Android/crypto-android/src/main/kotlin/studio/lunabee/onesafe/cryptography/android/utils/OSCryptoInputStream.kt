...
/**
 * An input stream which wraps crypto errors to [OSCryptoError]
 */
class OSCryptoInputStream(
    cryptoStream: InputStream,
) : FilterInputStream(cryptoStream) {
    override fun read(): Int = safeRead {
        super.read()
    }

    override fun read(b: ByteArray?, off: Int, len: Int): Int = safeRead {
