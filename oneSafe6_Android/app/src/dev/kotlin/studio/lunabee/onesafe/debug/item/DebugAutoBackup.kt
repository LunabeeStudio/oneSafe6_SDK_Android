...
class DebugAutoBackup(
    ...
    private val allSafeId: List<SafeId>,
    ...
    fun Composable(modifier: Modifier) {
        ...
        OSRow(
            ...
            DropdownMenu(
                ...
                OSDropdownMenuItem(text = LbcTextSpec.Raw("Trigger auto-backup for every safe"), null) {
                    allSafeId.forEach { safeId ->
                        val inputData = Data.Builder()
                            .putByteArray(AutoBackupChainWorker.AUTO_BACKUP_CHAIN_WORKER_SAFE_ID_DATA, safeId.toByteArray())
                            .build()
                        workManager.enqueue(
                            OneTimeWorkRequestBuilder<AutoBackupChainWorker>()
                ...
                safeId?.let {
                    val inputData = Data.Builder()
                        .putByteArray(AutoBackupChainWorker.AUTO_BACKUP_CHAIN_WORKER_SAFE_ID_DATA, safeId.toByteArray())
                        .build()
                    OSDropdownMenuItem(text = LbcTextSpec.Raw("Trigger auto-backup for current safe only"), null) {
                        workManager.enqueue(
                            OneTimeWorkRequestBuilder<AutoBackupChainWorker>()
                                .setInputData(inputData)
                                .build(),
