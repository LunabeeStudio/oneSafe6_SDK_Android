...
class LegacyLocalCtaStateTest {
    ...
    fun deserialize_test() {
        ...
        // expected
        val hidden = LegacyLocalCtaState.Hidden
        val visibleSince123 = LegacyLocalCtaState.VisibleSince(123)
        val dismissedAt456 = LegacyLocalCtaState.DismissedAt(456)

        assertDoesNotThrow {
            val hiddenDecoded = ProtoBuf.decodeFromHexString<LegacyLocalCtaState>(hiddenEncoded)
            assertEquals(hidden, hiddenDecoded)

            val visibleSince123Decoded = ProtoBuf.decodeFromHexString<LegacyLocalCtaState>(
                visibleSince123Encoded,
            )
            assertEquals(visibleSince123, visibleSince123Decoded)

            val dismissedAt456Decoded = ProtoBuf.decodeFromHexString<LegacyLocalCtaState>(
                dismissedAt456Encoded,
            )
    ...
    fun serialize_map_test() {
        ...
        val expectedMap = mapOf(
            "cta1" to LegacyLocalCtaState.Hidden,
            "cta2" to LegacyLocalCtaState.VisibleSince(1234L),
        )
        val actual: String = ProtoBuf.encodeToHexString(
            LegacyLocalCtaStateMap(
                expectedMap,
            ),
        )
        assertEquals(expected, actual)
        val actualMap = ProtoBuf.decodeFromHexString<LegacyLocalCtaStateMap>(actual)
