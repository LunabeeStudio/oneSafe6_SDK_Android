...
import studio.lunabee.onesafe.test.toRoomUpdateSafeItem
...
class MainDatabaseTest {
    ...
    fun insertAndGetSafeItem() {
        val roomSafeItem: RoomSafeItem = CommonTestUtils.roomSafeItem()
        runTest {
    ...
    fun insertAndFindSafeItemById() {
        val safeItem: RoomSafeItem = CommonTestUtils.roomSafeItem(position = 1.0)
        var lastIdx = 0
        runTest {
    ...
    fun cascadeDeleteItems() {
        val parentSafeItem: RoomSafeItem = CommonTestUtils.roomSafeItem()
        val childSafeItem: RoomSafeItem = CommonTestUtils.roomSafeItem(parentId = parentSafeItem.id)
        runTest {
    ...
    fun recursive_triggers_test(): TestResult = runTest {
        ...
        val item0: RoomSafeItem = CommonTestUtils.roomSafeItem(id = id0, parentId = id0)
        val item1: RoomSafeItem = CommonTestUtils.roomSafeItem(id = id1, deletedParentId = id1)
        val item2: RoomSafeItem = CommonTestUtils.roomSafeItem(id = id2)
