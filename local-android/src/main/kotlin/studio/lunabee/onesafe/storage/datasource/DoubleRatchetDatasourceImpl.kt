 ...
 * Last modified 30/06/2023 09:41
 */
...
class DoubleRatchetDatasourceImpl @Inject constructor(
    private val doubleRatchetConversationDao: DoubleRatchetConversationDao,
    ...
    override suspend fun getConversation(id: DoubleRatchetUUID): Conversation? {
        val key = contactKeyRepository.getContactLocalKey(id.uuid)
        return doubleRatchetConversationDao.getById(id.uuid)?.toEncConversation()?.let { encConversation ->
    ...
    override suspend fun popMessageKey(id: DRMessageKeyId): DRMessageKey? {
        val key = doubleRatchetKeyDao.getById(id.value)?.let { encData ->
            val key = contactKeyRepository.getContactLocalKey(UUID.fromString(id.conversationId))
            DRMessageKey(bubblesCryptoRepository.localDecrypt(key, DecryptEntry(encData, ByteArray::class)))
        }
        doubleRatchetKeyDao.deleteById(id.value)
        return key
    }

    override suspend fun saveMessageKey(id: DRMessageKeyId, key: DRMessageKey) {
        val contactKey = contactKeyRepository.getContactLocalKey(UUID.fromString(id.conversationId))
        val encDoubleRatchetKey = EncDoubleRatchetKey(
            id = id.value,
            data = bubblesCryptoRepository.localEncrypt(contactKey, EncryptEntry(key.value)),
        )
        return doubleRatchetKeyDao.insert(RoomDoubleRatchetKey.fromEncDoubleRatchetKey(encDoubleRatchetKey))
    }

    override suspend fun saveOrUpdateConversation(conversation: Conversation) {
        val key = contactKeyRepository.getContactLocalKey(conversation.id.uuid)
        ...
        return doubleRatchetConversationDao.insert(RoomDoubleRatchetConversation.fromEncConversation(encConversation))
    }
}
