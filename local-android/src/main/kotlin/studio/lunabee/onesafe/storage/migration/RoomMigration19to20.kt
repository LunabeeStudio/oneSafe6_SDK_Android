...
class RoomMigration19to20 @Inject constructor() : Migration(19, 20) {

    /**
     * Makes [RoomSafeCrypto.encBubblesKey] not null
     */
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        // Insert default encBubblesKey if it is null
        val cursor = db.query("SELECT $Id FROM Safe_Old WHERE $CryptoEncBubblesKey IS NULL")
        while (cursor.moveToNext()) {
            val values = ContentValues().apply {
                put(CryptoEncBubblesKey, byteArrayOf(0))
            }
            db.update("Safe_Old", SQLiteDatabase.CONFLICT_FAIL, values, "$Id = ?", arrayOf(cursor.getBlob(0)))
        }
