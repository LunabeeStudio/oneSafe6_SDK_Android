...
private val log = LBLogger.get<LocalSignInWithBiometricUseCase>()

class LocalSignInWithBiometricUseCase @Inject constructor(
    ...
    suspend operator fun invoke(cipher: Cipher): LBResult<Unit> = OSError.runCatching(log) {
        cryptoRepository.loadMasterKeyFromBiometric(cipher)
    }
}
