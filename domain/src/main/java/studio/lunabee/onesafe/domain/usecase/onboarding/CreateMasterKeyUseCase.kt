...
private val log = LBLogger.get<CreateMasterKeyUseCase>()

/**
 * Create salt and masterKey from password, and keep it in memory
 */
class CreateMasterKeyUseCase @Inject constructor(
    ...
    suspend operator fun invoke(password: CharArray): LBResult<Unit> {
        return OSError.runCatching(log) {
            cryptoRepository.generateCryptographicData(password)
        }
    }
}
